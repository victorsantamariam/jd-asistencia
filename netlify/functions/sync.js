...
exports.handler = async function(event, context) {
  ...
  const authKey = (event.headers && (event.headers['x-access-key'] || event.headers['X-Access-Key'])) || 
  ...
  if (authKey !== 'MiMejorElemnto' && authKey !== 'MiMejorElemento') {
    return {
      statusCode: 401,
      headers,
      body: JSON.stringify({ success: false, error: 'Acceso no autorizado. Contraseña requerida.' })
    };
  }
  ...
  try {
    ...
    const parametros = {
      niveles: [
        { nivel: '🔴 CRÍTICO', hasta: 0.000, prioridad: 1, descripcion: '0% de asistencia', accion: 'Contactar al estudiante' },
        { nivel: '🟠 ALERTA', hasta: 0.033, prioridad: 2, descripcion: 'Más de 0% y hasta 3.3%', accion: 'Contactar al estudiante' },
        { nivel: '🟡 SEGUIMIENTO', hasta: 0.067, prioridad: 3, descripcion: 'Más de 3.3% y hasta 6.7%', accion: 'Monitorear' },
